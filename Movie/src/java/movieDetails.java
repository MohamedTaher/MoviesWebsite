 ...
 * @author LOL
 */
@WebServlet(urlPatterns = {"/movieDetails"})
        ...
        response.setContentType("text/html;charset=UTF-8");
            try( PrintWriter out = response.getWriter())
            {
                ...
                else
                {
                        ...
                        ArrayList<MovieItem> list = MovieItem.selectByID(id);
                        ArrayList<MovieItem> trailer = MovieItem.getTrailers(id);
                        ArrayList<MovieItem> Categorys = MovieItem.getCategorys(id);
                        request.setAttribute("listt", list);
                        request.setAttribute("trailer", trailer);
                        request.setAttribute("Categorys", Categorys);
//         ServletContext context = getServletContext( );
//        context.log("hhh "+Categorys.get(0).getNameOfcategory());
